...
> Builds on Ch 24 (patterns), Ch 23 (a11y), Ch 18 (forms). The "design a Button/Modal/Table"
> question — tests whether you think about API surface, composition, edge states, and
...
prop is a potential expansion point — prefer variant/enum over boolean for visual options.
...
- **Polymorphism**: Button should render as `<button>`, `<a>`, or `<div>` based on `as` prop
  or `href` presence — without losing accessibility.
...
- **Form submit**: `type="submit"` must trigger form `onSubmit` — don't prevent this.
...
extra logic. Pattern is usable in ButtonGroup, Tabs, Accordion, Modal, Table selection — any
...
| Virtualization toggle | Table doesn't force one approach — but opts in |
| Loading/empty/error states | Every data component needs these (Ch 33) |
...
Not just Toast component — a full system.
...
1. **Who owns the state?** — Controlled, uncontrolled, or both?
...
7. **Is it accessible?** — Role, aria-* attributes, focus management, screen reader.
8. **Does it need a ref?** — `forwardRef` for focus, measure, form integration.
9. **Does it need a portal?** — Modal, Dropdown, Tooltip, Toast.
10. **Does it need animation?** — Enter/exit transitions (Framer Motion, CSS transitions).
11. **Can I make it composable?** — `renderX` slots, `as` prop, children-based extension.

---

## Summary

> **Design the API before writing the implementation. The API is the contract with every**
> **consumer of your component. A strong API makes correct usage easy and incorrect usage**
> **hard. Forward refs, support controlled + uncontrolled, use enums over booleans, and**
...
1. Design a DatePicker component — write the full TypeScript API contract (no implementation)
