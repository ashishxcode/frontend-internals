...
The most important pattern for component library design. Parent manages state, children consume it implicitly via Context:
...
Tabs.Trigger = function TabsTrigger({ value, children, ...props }) {
...
Tabs.Content = function TabsContent({ value, children, ...props }) {
  const ctx = useContext(TabsContext);
...
- Consumer doesn't need to manage state (parent does)
- Internal state is implicit (no prop drilling)
- Easy to extend (add new sub-components without breaking API)
- Accessible by default (ARIA roles baked in)
