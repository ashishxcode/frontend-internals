...
File uploads are a common machine-coding prompt. The key challenges: drag-and-drop, preview, progress tracking, and validation.
...
Use `XMLHttpRequest` for progress tracking — `fetch` doesn't support upload progress events.

### Validation

Validate before upload: file type (MIME or extension), file size (prevent huge files), and file count (single vs multiple). Show errors immediately — don't upload and let the server reject.

### Multi-Step/Wizard Forms

Multi-step forms need state persistence across steps:
...
function WizardForm() {
  ...
  const steps = [PersonalInfo, Address, Payment];
...
**Key decisions:**
- Store step data in parent state (not URL, unless you need shareable links)
- Validate each step before allowing "Next"
- Allow going back without losing data
- Submit all data at once on the final step
