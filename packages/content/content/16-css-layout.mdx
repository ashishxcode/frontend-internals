...
Modern responsive design uses three tools:
...
Instead of querying the viewport, query the parent container:
...
The card layout adapts to its container width, not the viewport. This is critical for component libraries where a card might live in a sidebar (narrow) or main content (wide).
...
No media queries needed. The font scales smoothly between min and max.
...
.button { background: var(--primary); padding: var(--spacing-md); }
```

**Performance:** Custom properties are resolved at computed-value time, not parse time. They inherit down the DOM tree. Changing a variable on a parent triggers re-computation of all descendants — powerful for theming but can cause performance issues if overused.
...
**vs Tailwind:** Tailwind generates static utility classes at build time. Custom properties are dynamic at runtime. They solve different problems — Tailwind for layout/spacing consistency, custom properties for runtime theming.
