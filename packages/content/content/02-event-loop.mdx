...
Picture this: you're building a CRM dashboard. A sales rep clicks on an account, and you need to fetch the account details, the contact list, and the deal pipeline — all from different API endpoints. Each one takes 200-400ms.

If JavaScript blocked the thread for each request, the page would freeze for over a second. No scrolling. No clicking. No loading spinners. The user would see that dreaded "page unresponsive" dialog.

Here's the thing — JavaScript runs on a single thread. One call stack. It can't "wait" for anything without freezing everything. So how do you handle slow operations like network requests without locking up the whole page?
...
Before JavaScript needed to handle async work, languages took two approaches — and neither worked for UI.

**Synchronous blocking (C, Java, Python default):** A `read()` call blocks the thread until data arrives. The OS switches to another thread. But browsers give each tab one UI thread. Block it, and the entire tab freezes. The user can't scroll, click, or type.

**Thread-per-request (Java servlets, C# servers):** Each connection spawns a new thread. The OS handles blocking by switching between threads. But threads are expensive — each needs megabytes of stack space. A browser tab with 100 open network requests would crash on thread overhead alone. Plus, threads bring race conditions, deadlocks, and complexity that makes UI programming a nightmare.

**The old web was static.** Before JavaScript, clicking a link reloaded the entire page. No interactivity. The browser was a document viewer, not an app platform.

JavaScript needed a third way: **single thread, cooperative scheduling, non-blocking I/O.** The language never waits. The host (browser) does the waiting on its own threads. When the host finishes, it drops a callback into a queue. The single thread picks it up when it's free.

The insight is simple: instead of making developers manage threads, the runtime manages a queue of work. Developers write callbacks. The runtime decides when to run them. No locks. No race conditions. One thread, one stack, one queue at a time.

## Mental Model — One Core Insight

Here's the restaurant analogy that makes everything click:

**The event loop is like a restaurant host.** You (the JavaScript thread) are the only waiter. You can only carry one plate at a time (one function on the call stack). When you take an order (call a function), you do it yourself — no one else can help.

But here's the key: some orders take a long time. Cooking a steak (network request), waiting for the bar to make a cocktail (timer), or checking with the kitchen about allergies (DOM event). You don't stand there waiting. You hand those off to the kitchen staff (browser host threads). They do the work in the background.

When the kitchen finishes, they don't interrupt you mid-plate. They put your completed order in a window. Actually, they put it in *two* windows:
...
- **Window 2 (Macrotasks):** Regular orders. setTimeout callbacks, click handlers, I/O. You pick one of these after the VIP window is completely empty.

Your only job: finish your current plate, check the VIP window (drain ALL microtasks), maybe wipe down the counter (paint), then pick one regular order. Repeat forever.
...
## Engine Simulation — Try This Yourself

Run this in your head before looking at the answer:
...
Step 3: Promise.resolve().then(() => console.log("3"))
  ...
  .then() schedules the callback as a microtask.
...
The HTML spec defines the event loop processing model. Here's the pseudo code:
...
Here's what the event loop does:
...
Microtasks are high-priority callbacks that the event loop drains completely after each macrotask. Examples include `Promise.then` reactions, `async/await` continuations, and `queueMicrotask` callbacks. Macrotasks are normal-priority callbacks that the event loop picks one at a time from the task queue. Examples include `setTimeout` callbacks, DOM event handlers, I/O callbacks, and `setInterval` ticks. The critical difference in behavior: after each macrotask, the event loop runs `performMicrotaskCheckpoint()`, which loops until the microtask queue is empty. Only then does it consider rendering and the next macrotask. This means a microtask that enqueues another microtask keeps the loop running — macrotasks and paint frames are starved until the microtask queue is truly empty.

**Q2: Why does Promise.then(fn) run before setTimeout(fn, 0) even when the setTimeout appears first in the code?**

setTimeout schedules its callback as a macrotask. Promise.then schedules its callback as a microtask. After synchronous code finishes (the call stack empties), the event loop performs a microtask checkpoint — draining all microtasks before picking the next macrotask. The Promise callback is in the microtask queue and runs immediately. The setTimeout callback sits in the macrotask queue and waits. Even with a 0ms delay, setTimeout is clamped to 4ms for nested timers (HTML spec §8.6) and does not fire until after the microtask drain. The order in source code is irrelevant — what matters is which queue the callback lands in. This is why senior engineers simulate queues mentally instead of reading code top-to-bottom. See Ch 03 for how React uses this exact mechanism for automatic batching.

**Q3: An async function calls await on a non-promise value like await 5. Does this still cost a microtask tick? Explain why.**

Yes. Internally, `await` desugars to `Promise.resolve(value).then(resume, throwIn)`. Even when `value` is `5`, `Promise.resolve(5)` returns an already-fulfilled Promise. But `.then()` on a fulfilled Promise still enqueues a microtask — the spec requires this (ECMA-262 §25.6.5.2). The function suspends at the await point, returns control to the caller, and the continuation runs as a microtask on the next drain. This means `await null`, `await undefined`, `await 5`, and `await Promise.resolve(1)` all cost at least one microtask tick. The function yields and resumes asynchronously regardless of the awaited value's type. This is why `async` functions always return to the caller before continuing after an await — there is no synchronous fast path for non-Promise values.

**Q4: A while(true) loop runs for 3 seconds. Why does the UI freeze, and what two approaches fix it?**

The `while(true)` loop keeps the call stack busy for 3 seconds. The event loop cannot run its tick because the stack is never empty (Ch 01 — the stack must be empty for the event loop to dequeue callbacks). No macrotasks run (click handlers, timer callbacks, I/O). No microtasks run (Promise reactions). No `requestAnimationFrame` callbacks fire. No paint frames render. The browser's compositor thread is still running, but it can only composite whatever was last painted — the user sees a frozen frame.

Fix one: **chunk with setTimeout**. Break the work into small pieces (e.g., 1000 iterations per chunk) and schedule each chunk with `setTimeout(..., 0)`. Each chunk runs as a macrotask. Between chunks, the stack empties, the event loop drains microtasks, and the browser paints. The user sees incremental progress.

Fix two: **Web Worker**. Move the computation to a separate thread. The worker runs `while(true)` independently. It communicates results back via `postMessage`, which enqueues a macrotask on the main thread. The main thread stays responsive — it can handle clicks, scroll, and paint while the worker crunches numbers. See Ch 17 for Worker communication patterns and Ch 08 for the "do it elsewhere" performance strategy.
...
The algorithm (from the HTML spec): set a boolean flag `isPerformingMicrotaskCheckpoint`. If the flag is already true, return immediately (re-entrancy guard). Otherwise, set it to true. Then loop: while the microtask queue is not empty, dequeue the oldest microtask and run it. After the loop, set the flag to false.

The re-entrancy guard prevents recursive checkpoints. If a microtask triggers an operation that would normally start a new checkpoint (like resolving a Promise inside a Promise reaction), the nested call returns immediately. Without the guard, you could get infinite recursion: checkpoint runs microtask → microtask resolves Promise → triggers another checkpoint → runs the same microtask again. The guard ensures only one checkpoint runs at a time, and the outer loop handles any newly queued microtasks.

If a microtask queues another microtask, the while loop keeps going. It only exits when the queue is truly empty. This means an infinite chain of `Promise.then` calls (where each callback creates and resolves another Promise) keeps the loop running forever. Macrotasks never get a turn. Paint never happens. The page freezes even though each individual microtask is tiny. This is the "microtask storm" failure mode. See Ch 03 for how React's microtask-based batching avoids this by scheduling one microtask per root per event, not an unbounded chain.
...
- Trigger: "Empty stack, drain the VIPs, serve one regular order, repeat."
