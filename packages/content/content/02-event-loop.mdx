...
When you need to run multiple async operations, Promise provides four combinators:
...
Resolves when ALL promises resolve. If ANY rejects, the whole thing rejects immediately. Use when all results are needed and one failure means the operation fails.
...
Resolves when ALL promises settle (either resolve or reject). Never rejects. Returns an array of `{ status, value/reason }`. Use when you want all results regardless of individual failures.
...
Resolves or rejects with the FIRST promise to settle. The other promises continue running but their results are ignored. Use for timeout patterns:
...
Resolves with the FIRST successfully resolved promise. Ignores rejections until ALL reject (then throws `AggregateError`). Use when you have multiple sources and any one succeeding is fine.
...
| `Promise.allSettled` | All settle | Never | Report all results |
| `Promise.race` | First settles | First rejects | Timeout pattern |
| `Promise.any` | First resolves | All reject | Fallback sources |

## `requestAnimationFrame` — Where It Fits

rAF is neither a microtask nor a macrotask. It sits between macrotask execution and browser paint:
...
rAF callbacks run once per frame (~16.67ms at 60fps). They fire after microtasks but before paint. This guarantees your DOM mutations happen right before the browser paints — no wasted frames.

**Interview gotcha:** `setTimeout(fn, 0)` and `requestAnimationFrame(fn)` are not the same. `setTimeout` schedules a macrotask that may run at any time after the current task. `rAF` runs before the next paint, giving you frame-synchronized updates.
