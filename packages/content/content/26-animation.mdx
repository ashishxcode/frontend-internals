...
`transition` defines what properties animate, how long, and the easing. When the property changes (hover, class toggle), the browser interpolates smoothly.

### Keyframe Animations — Continuous Motion
...
`@keyframes` defines the animation stages. `animation` applies it with duration, timing, iteration count.

### Timing Functions

| Function | Behavior | Use case |
|---|---|---|
| `ease` | Slow start, fast middle, slow end | Default, natural feel |
| `linear` | Constant speed | Progress bars, rotations |
...
| `cubic-bezier(x1, y1, x2, y2)` | Custom curve | Exact control |

### Spring Physics (Framer Motion)

Springs feel more natural than easing curves because they model real physics:
...
- **Mass:** How heavy the object. Higher = slower, more momentum.

Springs automatically adjust duration based on distance — short moves are fast, long moves are slow. Easing curves have fixed duration regardless of distance.
