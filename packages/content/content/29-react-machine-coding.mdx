...
> Prerequisites: React rendering and reconciliation (how React diffs the virtual DOM and commits updates), hooks (useState, useEffect, useRef, useCallback. These are the primitives for stateful function components), key prop (how React identifies elements across renders, critical for lists and form inputs), and component patterns (controlled vs uncontrolled, lifting state up, render props).
...
> **working UI → edge cases → accessibility in 15 to 20 minutes, narrating your reasoning?**
...
From "minimal state → working UI → edges" you learn: use `useState` before over-engineering,
discriminated unions for status, controlled vs uncontrolled choice, keyboard + screen reader
...
- Keyboard: ArrowUp and ArrowDown to navigate, Enter to select, Escape to close
...
- `IntersectionObserver` performs better than scroll listeners. Scroll listeners fire on every pixel scroll (60+ times/sec) on the main thread. IntersectionObserver is browser-native and callback-driven. It only fires when observed elements cross the viewport boundary.
...
- **Key**: Use item ID, not index. Indexes shift when items are filtered or sorted.
- **Tab navigation**: Focusable elements inside virtual rows need special handling.
  Make sure focus does not jump unexpectedly.
- **Libraries**: `react-window` (fixed), `react-virtuoso` (dynamic), `@tanstack/react-virtual`.

### Real-world

"I would use `@tanstack/react-virtual` for a production virtual list. It handles
dynamic heights, sticky headers, scroll-to-index, and RTL. But for the interview, I would
