...
> Prerequisites: performance optimization (lazy loading, code splitting, React.memo, useMemo, useCallback), data fetching patterns (loading states, retry logic, stale-while-revalidate via TanStack Query), architecture tradeoffs (client vs server rendering, state management decisions), and error handling (try/catch, error boundaries, user-facing error messages, logging/reporting strategies).
> The JD repeatedly says "think about special cases, loading
...
> **An SDE-2 does not just handle the happy path. They mentally picture every state the UI**
> **can be in and make sure each one is designed, not an afterthought.**

From "intent → flight → result" you learn: the four states (loading, empty, error, data),
optimistic updates for perceived speed, undo for error recovery, retry for temporary failures,
...
2. Design for all four states: loading, empty, error, data. Each needs its own UI treatment
...
| No notifications | Bell icon + "All caught up" | None |
...
goes wrong. TanStack Query's `onMutate`, `onError`, and `onSettled` lifecycle makes this
